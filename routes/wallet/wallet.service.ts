...
import {
  ...
  recordTopUpSuccess,
  getUserBankInfo,
  updateBankInfo as updateBankInfoQuery,
  moveToPendingWithdrawal,
  deductPendingWithdrawal,
  getUserById,
...
import {
  ...
  getWithdrawalById,
  updateWithdrawalStatus as updateWithdrawalStatusQuery,
} from "../../db/withdrawals/queries.js";
import {
  WithdrawalStatus,
  type WithdrawalRequest,
} from "../../db/withdrawals/domain.js";
...
import {
  ...
  NotificationChannel,
} from "../../db/notifications/domain.js";
...
export async function getUserBalancesService(userId: string) {
  ...
  return (
    balances || { availableBalance: 0, frozenBalance: 0, pendingWithdrawal: 0 }
  );
}

export async function syncTransactionStatusService(
  userId: string,
  orderCode: number,
) {
  const pool = getPool();
...
export async function requestWithdrawalService(
  ...
  if (!Number.isInteger(amountCoin) || amountCoin < MIN_WITHDRAWAL) {
    throw new DomainValidationError(
      `Minimum withdrawal is ${MIN_WITHDRAWAL} coins, and must be an integer`,
    );
  }
  ...
  if (
    ...
    throw new DomainValidationError(
      "Bank information is required before withdrawing",
    );
  }
...
export async function getBankInfoService(userId: string) {
  ...
  return (
    info || { bankName: null, bankAccountNumber: null, bankAccountHolder: null }
  );
}
...
export async function confirmWithdrawalService(withdrawalId: string) {
  ...
  try {
    ...
    await updateWithdrawalStatusQuery(
      client,
      withdrawalId,
