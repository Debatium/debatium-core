  ...
  return {
    Redis: vi.fn().mockImplementation(function() {
      return {
        ...
        set: vi.fn(async (key: string, value: any) => { redisStore.set(key, value); }),
...
export async function createAuthenticatedUser(options: { 
  ...
  const email = options.email || `test_${Math.random().toString(36).substring(2)}@example.com`;
  const password = options.password || 'TestPassword123@';
  const fullName = options.fullName || 'Test User';
  const username = options.username || `testuser_${Math.random().toString(36).substring(7)}`.replace(/[^a-zA-Z0-0_]/g, '_');
